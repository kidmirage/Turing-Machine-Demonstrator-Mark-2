...
class VirtualKeyboard():
    ...
    def __init__(self, screen):
        ...
        self.keyW = int(self.w/13+0.5) # key width with border
        self.keyH = int(self.h/7+0.5) # key height
...
#        self.textbox = pygame.Surface((self.rect.width,self.keyH*2))
    ...
    def run(self, text=''):
        ...
        self.input = TextInput(self.screen,self.text,self.x,self.y+22,self.textW,self.textH)
...
##            gtk.main_iteration(block=False)
    ...
    def clickatmouse(self):
        ...
        self.unselectall()
        for key in self.keys:
            keyrect = pygame.Rect(key.x,key.y,key.w,key.h)
...
#            print 'input {}'.format(pos)
    ...
    def addkeys(self):  # Add all the keys for the virtual keyboard 
        ...
        x = self.x
        y = self.y + self.textH + self.keyH/2
...
#        print 'addkeys keyW {} keyH {}'.format(self.keyW, self.keyH)
        ...
        xfont = pygame.font.SysFont('arial', 22, bold=True) # I like this X better #TODO???
        onekey = VKey('X',self.x+self.textW+20,self.y,self.keyW,self.keyH,xfont) # exit key TODO???
        onekey.special = True
        onekey.escape = True
        self.keys.append(onekey)
...
class TextInput():
    ...
    def __init__(self, screen, text, x, y, w, h):
        ...
        self.background.fill((WHITE)) # fill with black
...
#        self.font = pygame.font.Font(None, fontsize) # use this if you want more text in the line
        ...
        self.txtFont = pygame.font.SysFont('arial', fsize, bold=True)
...
#        print 'txtinp: width={} rtX={} font={} lineChars={} lineH={}'.format(self.w,rtX,fsize, self.lineChars,self.lineH)
        ...
        self.cursorlayer = pygame.Surface((22,4)) # thin vertical line
        self.cursorlayer.fill((PURPLE)) # white vertical line
        self.cursorvis = True
...
#        self.layer.fill([255, 255, 255, 127]) # 140
        self.layer.fill((WHITE)) # clear the layer
        pygame.draw.rect(self.layer, (DARK_PURPLE), (0,0,self.w,self.h), 3) # draw the box

# should be more general, but for now, just hack it for 2 lines
        txt1 = self.text[:self.lineChars] # line 1
        t1 = self.txtFont.render(txt1, 1, (BLACK)) # line 1
        self.layer.blit(t1,(4,8))
    ...
    def drawcursor(self):
        ...
        if self.cursorpos > 0:
            linetext = self.text[0:self.cursorpos]
            rtext = self.txtFont.render(linetext, 1, (PURPLE))
...
#        print 'text {}'.format(self.text)
        while p < l:
            text = self.txtFont.render(self.text[:p+1], 1, (PURPLE)) # how many pixels to next char?
            rtext = text.get_rect()
...
##        self.keylayer.set_alpha(160)
        ...
        pygame.draw.rect(self.keylayer, (BLACK), (0,0,self.w,self.h), 3)
    ...
    def draw(self, screen, background, shifted=False, forcedraw=False):
        ...
        position = pygame.Rect(self.x, self.y, self.w, self.h)
        ...
        pygame.draw.rect(self.keylayer, color, (3,3,self.w-6,self.h-6))
        ...
        color = (DARK_PURPLE)
        text = self.font.render(keyletter, 1, color)
